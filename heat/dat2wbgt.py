...
"""
dat2wbgt.py
Read a 15-min csv file for one tower site, calculate the wet bulb globe
  temperature for each time, write a simple csv file.
  Read the data from a dat file, rather than data request, WMDC, etc.
Usage: python dat2wbgt.py [-v] [-o csvfile] metfile  
...
parser.add_argument("-v", "--verbosity", help="Increase the verbosity of the output",
                    action="count")
parser.add_argument("-o", "--csvfile", help="Specified name of the CSV file that will be written, otherwise it will be dat2wbgt.csv")
...
if args.csvfile:
    ...
    csvFile = 'dat2wbgt.csv'  # Default csv file name.
...
columnDateTime1 = 'datetime'   # Data request from current Data Requests
#columnDateTime2 = 'dts'        # Data request from current Weather Machine.
columnDateTime2 = 'TIMESTAMP'  # From a data logger dat file.
columnName = {}
#columnName['spd1'] = 'spd1'
columnName['spd1'] = 'HWSpd1_AVGms'
#columnName['temp0'] = 'temp0'
columnName['temp0'] = 'AirTemp1_AVGDegC'
#columnName['rh'] = 'rh'
columnName['rh'] = 'RotronRH_AVGpercent'
#columnName['dewp'] = 'dewp'
columnName['dewp'] = 'DewPt_AVGDegC'
#columnName['precip'] = 'precip'
columnName['precip'] = 'Rain_TOTinch'
#columnName['press'] = '
columnName['press'] = 'BaroPres_AVGmbar'
#columnName['swdn'] = 'swdn'
columnName['swdn'] = 'SWUP_AVGWm2'
...
with open(metFile, 'r') as infile:
    ...
    for row in towerData:
        ...
        if (row[columnDateTime] and
            ...
            try:
                T2mC = float(row[columnName['temp0']])
            except ValueError:
                T2mC = None
            try:
                Td2mC = float(row[columnName['dewp']])
            except ValueError:
                Td2mC = None
            try:
                wspd10m = float(row[columnName['spd1']])
            except ValueError:
                wspd10m = None
            try:
                pSfc = 100.0 * float(row[columnName['press']])
            except ValueError:
                pSfc = None
            try:
